...
local error_codes_defined = {
	[200] = 'OK - Request Feita Com Sucesso..!',
	[204] = 'OK - No Content',
...
function sendDebugMessage(msg)
	print("^1[^5Dnzx - API^1] ^3" .. msg);
end

tracked = {}

RegisterNetEvent('Dnzx - API:PlayerLoaded')
AddEventHandler('Dnzx - API:PlayerLoaded', function()
	if (GetCurrentResourceName() ~= "Dnzx - API") then 
		TriggerClientEvent('chatMessage', -1, '^1[^5SCRIPT ERROR^1] ^3The script ^1' .. GetCurrentResourceName() .. ' ^3will not work properly... You must '
	.. 'rename the resource to ^1Dnzx - API');
	end
	...
	if (tracked[license] == nil) then 
		...
		'^1[^5Dnzx - API^1] ^3The Discord API script was created by Badger. You may join his Discord at: ^6discord.gg/WjB5VFz')
	end
	TriggerClientEvent('chatMessage', source, 
...
function IsDiscordEmailVerified(user) 
    ...
    if discordId then 
        ...
        if member.code == 200 then
        	...
        	sendDebugMessage("[Dnzx - API] ERROR: Code 200 was not reached. DETAILS: " .. error_codes_defined[member.code]);
        end
    end
    return isVerified;
end
...
function GetGuildIcon(guild --[[optional]])
  local guildId = GetGuildId(guild)
	...
	if guild.code == 200 then
		...
		sendDebugMessage("[Dnzx - API] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data or guild.code)) 
	end
	return nil;
end
...
function GetDiscordAvatar(user)
	...
	if discordId then 
		...
		sendDebugMessage("[Dnzx - API] ERROR: Discord ID was not found...")
	end
    return imgURL;
end
...
function GetDiscordRoles(user, guild --[[optional]])
  ...
  local roles = nil
	...
	if discordId then
		...
		sendDebugMessage("[Dnzx - API] ERROR: Discord was not connected to user's Fivem account...")
		return false
	end
	return false
end
...
function GetUserRolesInGuild (user, guild)
	...
	if not user then
		sendDebugMessage("[Dnzx - API] ERROR: GetUserRolesInGuild requires discord ID")
		return false
	end
	if not guild then
		sendDebugMessage("[Dnzx - API] ERROR: GetUserRolesInGuild requires guild ID")
		return false
	end
	...
	if member.code == 200 then
		...
		sendDebugMessage("[Dnzx - API] ERROR: Code 200 was not reached... Returning false. [Member Data NOT FOUND] DETAILS: " .. error_codes_defined[member.code])
		return false
	end
end
  ...
  local guildId = GetGuildId(guild)
	...
	if discordId then
		...
		if member.code == 200 then
			...
			sendDebugMessage("[Dnzx - API] ERROR: Code 200 was not reached. Error Code: " .. error_codes_defined[member.code])
			return nil;
		end
	else
		sendDebugMessage("[Dnzx - API] ERROR: Discord was not connected to user's Fivem account...")
...
Citizen.CreateThread(function()
  ...
  if mguild.code == 200 then
    ...
    sendDebugMessage("[Dnzx - API] Conexão Feita Com O Resource [discord] : "..data.name.." ("..data.id..")")
  else
    sendDebugMessage("[Dnzx - API] An error occured, please check your config and ensure everything is correct. Error: "..(mguild.data and json.decode(mguild.data) or mguild.code)) 
  end
  if (Config.Multiguild) then 
    for _,guildID in pairs(Config.Guilds) do
      ...
      if guild.code == 200 then
        local data = json.decode(guild.data)
        sendDebugMessage("[Dnzx - API] Successful connection to Guild : "..data.name.." ("..data.id..")")
      else
        sendDebugMessage("[Dnzx - API] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data and json.decode(guild.data) or guild.code)) 
      end
    end
  end
end)
