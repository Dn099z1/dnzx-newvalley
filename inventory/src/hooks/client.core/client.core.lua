...
function Creative.Parachute()
	...
	SetPedParachuteTintIndex(Ped, 2)
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- FISHCOORDS
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	while true do
		...
		if LocalPlayer["state"]["Route"] < 900000 then
			...
			for _,v in pairs(Drops) do
				...
				if Distance <= 5 then
					TimeDistance = 1
					DrawMarker(3, v["Coords"][1], v["Coords"][2], v["Coords"][3] + 0.25, 0.0, 0.0, 0.0, 0.0, 180.0, 0.0, 0.1, 0.1, 0.1,255, 255, 0, 200, 0, 0, 0, 1)
...
RegisterNUICallback("requestInventory",function(Data,Callback)
	...
	if inventario then
		Callback({ inventario = inventario, drop = Items, invPeso = invPeso, invMaxpeso = invMaxpeso,phone = vSERVER.GetPhone(), name = vSERVER.GetName(), bank = vSERVER.GetBank(), avatar = vSERVER.GetPhoto() })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PICKUPITEM
